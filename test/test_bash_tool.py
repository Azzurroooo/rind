...
def test_echo(shell_tools) -> None:
    payload = assert_ok(parse_payload(run(shell_tools.bash("echo hello"))))
...
def test_cd_is_scoped_to_one_command(shell_tools, temp_dir: Path) -> None:
    ...
    payload = assert_ok(parse_payload(run(shell_tools.bash(in_dir_command, _session_id=sid))))
    data = payload.get("data") or {}
    stdout = (data.get("stdout") or "").strip()
    if temp_dir.name not in stdout:
        raise AssertionError(f"Expected command to run in {temp_dir}, got: {data}")

    payload = assert_ok(parse_payload(run(shell_tools.bash(cwd_command, _session_id=sid))))
    data = payload.get("data") or {}
    stdout = (data.get("stdout") or "").strip()
...
def test_missing_cd_is_reported_by_shell(shell_tools, temp_dir: Path) -> None:
    ...
    payload = assert_ok(parse_payload(run(shell_tools.bash(command, _session_id=sid))))
    data = payload.get("data") or {}
    if data.get("status") != "failed" or not data.get("stderr"):
        raise AssertionError(f"Expected shell cd failure, got: {data}")

def test_forbidden_blocked(shell_tools) -> None:
    payload = assert_error(parse_payload(run(shell_tools.bash("shutdown -h now"))), "DangerousCommandBlocked")
    if payload.get("tool") != "bash":
        raise AssertionError(f"Expected tool=bash, got: {payload}")

def test_rm_allowed_by_default(shell_tools, temp_dir: Path) -> None:
    target = temp_dir / "delete-me.txt"
    target.write_text("delete", encoding="utf-8")
    session_id = f"test_rm_{temp_dir.name}"
    state = shell_tools.pool.get_state(session_id)
    if state.shell_backend == "powershell":
        command = f"rm -Force '{str(target).replace("'", "''")}'"
    else:
        command = f"rm -f {shlex.quote(target.as_posix())}"
    payload = assert_ok(parse_payload(run(shell_tools.bash(command, _session_id=session_id))))
    if payload.get("tool") != "bash":
        raise AssertionError(f"Expected tool=bash, got: {payload}")
    if target.exists():
