...
// Read-only workspace file tree (web-ui.md §1 / master plan §6.6 file tree P1):
  ...
  if (state.status === "error") {
    return (
      <div className="tree-state error indent" style={indent(depth)} role="alert">
        ...
        <button type="button" className="tree-retry" onClick={() => onRetry(path)}>Retry</button>
      </div>
    );
  }
  const children = sortEntries(state.entries);
  if (!children.length) {
    return <div className="tree-state indent" style={indent(depth)}>Empty directory</div>;
  }
  return (
    <>
      {children.map((entry) => {
    ...
    <div className="tree-preview" role="region" aria-label={`File preview ${preview.path}`}>
      ...
      {preview.status === "error" && (
        <div className="tree-state error" role="alert">
          <span>{preview.error || "Unable to read file"}</span>
          {preview.size != null && <small>{formatBytes(preview.size)}</small>}
        </div>
      )}
      {preview.status === "oversize" && (
        <div className="tree-state">
          <span>File too large ({formatBytes(preview.size)}), exceeds the 8 MiB preview limit</span>
        </div>
      )}
      {preview.status === "binary" && (
        <div className="tree-state">
          <span>Binary file ({preview.mime || "unknown type"}{preview.size != null ? ` · ${formatBytes(preview.size)}` : ""}), preview not supported</span>
        </div>
      )}
...
function errorMessage(error) {
  const text = String(error?.message || error || "Read failed");
  return text.replace(/^Runtime request failed\.$/, "Read failed");
}
