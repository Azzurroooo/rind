...
GUIDES: dict[str, ChannelGuide] = {
    ...
    "feishu": ChannelGuide(
        ...
        setup_steps=(
            "1) open.feishu.cn → 开发者后台 → 创建『企业自建应用』（名称/描述随意，个人版账号即可）",
            "2) 左侧『添加应用能力』→ 启用『机器人』（不启用这步，聊天输入框不会出现）",
            "3) 『凭证与基础信息』页 → 复制 App ID / App Secret 填到下面",
            "4) 『权限管理』→ 开通：获取用户发给机器人的单聊消息、接收群聊中@机器人消息、以机器人身份发消息、获取与上传图片或文件资源",
            "5) 『事件与回调』→ 订阅方式选『使用长连接接收事件』→ 添加事件『接收消息 im.message.receive_v1』（漏掉这步同样收不到消息）",
            "6) 『版本管理与发布』→ 创建版本并发布（自建应用自己审批通过后生效）",
        ),
        fields=(
