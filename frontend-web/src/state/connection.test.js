...
const authenticated = () => initialConnectionState({ authenticated: true });

describe("connection reducer — transition table (web-ui.md §2.1)", () => {
  it("starts unauthenticated in login", () => {
    expect(initialConnectionState().phase).toBe("login");
    expect(initialConnectionState({ authenticated: true }).phase).toBe("connecting");
  });
  ...
  it("reconnecting → connect_failed(attempt<3) → reconnecting", () => {
    const state = { ...authenticated(), phase: "reconnecting", needsSync: true };
  ...
  it("syncing → sync_start sets total and remaining", () => {
    const state = { ...authenticated(), phase: "syncing", needsSync: true };
  ...
  it("syncing → sync_progress counts remaining down and clamps at 0", () => {
    let state = { ...authenticated(), phase: "syncing", syncTotal: 5, syncRemaining: 5 };
  ...
  it("syncing → sync_complete → online and clears the gap", () => {
    const state = { ...authenticated(), phase: "syncing", needsSync: true, syncTotal: 4, syncRemaining: 1 };
  ...
  it("offline → retry → reconnecting with attempts reset", () => {
    const state = { ...authenticated(), phase: "offline", failedAttempts: 4, needsSync: true };
  ...
  it("offline → retry → socket_open → syncing (J2: retry walks through syncing)", () => {
    let state = { ...authenticated(), phase: "offline", failedAttempts: 3, needsSync: true };
  ...
  it("offline → socket_lost stays offline (no flicker while waiting)", () => {
    const state = { ...authenticated(), phase: "offline", failedAttempts: 3 };
    const next = reduceConnection(state, { type: "socket_lost" });
    expect(next.phase).toBe("offline");
  });
  ...
  it("login → socket_lost stays login (never connected)", () => {
    const next = reduceConnection(initialConnectionState(), { type: "socket_lost" });
    expect(next.phase).toBe("login");
  });

  it("any phase → sign_out → clean login state", () => {
    const state = { ...authenticated(), phase: "online", failedAttempts: 1 };
...
describe("connection reducer — scripted journey (J2)", () => {
  it("online → loss → reconnect attempts → offline → retry → syncing → online", () => {
    let state = { ...authenticated(), phase: "online" };
...
describe("connection reducer — invariants hold from every state", () => {
  ...
  it("never touches composer-owned state (draft/input live outside this reducer)", () => {
    ...
    const state = reduceConnection(authenticated(), { type: "socket_lost" });
