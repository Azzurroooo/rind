...
from agent.infrastructure.team import WorkspaceLock, initialize_team_project
...
@pytest.mark.asyncio
async def test_delegate_returns_busy_when_the_target_workspace_is_locked(tmp_path: Path, monkeypatch) -> None:
    project, _ = _project_with_researcher(tmp_path, monkeypatch)

    def builder(**kwargs):
        raise AssertionError("must not build a busy child")
    ...
    async with WorkspaceLock(project.project_id, "researcher"):
        result = json.loads(await delegator.delegate("researcher", "Inspect the workspace."))

    assert result["ok"] is False
    assert result["error_type"] == "WorkspaceBusy"
