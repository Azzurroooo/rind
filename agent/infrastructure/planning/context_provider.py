...
from agent.domain.planning import ensure_plan_defaults
...
class PlanContextProvider:
    ...
    def build_snapshot(self, char_limit: int = 1800) -> str:
        """Render the active plan for compaction handoff context."""
        try:
            plan = load_plan_if_exists()
        except Exception:
            return ""
        if not plan or plan.get("status") != "active":
            return ""
        ensure_plan_defaults(plan)
        return render_compact_plan_summary(plan, char_limit=max(0, int(char_limit))).strip()
